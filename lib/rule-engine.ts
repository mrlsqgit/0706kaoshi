...
const BUILTIN_NON_DATA_FIRST_CELL_KEYWORDS = [
  ...
  // 配送发货单/出库单 等单据型文件底部的「单据信息/尾注」行（首单元格标签）：
  // 即便 AI 漏设 footerRowsToSkip，也不会被当成 SKU 数据行污染。
  // 这些词本质上只会出现在尾注/单据信息行，不会是真实 SKU 数据行的首格
  // （已对全部 demo 文件全行扫描验证 0 误杀）。
  '单据号', '上游单据', '创建日期', '创建人', '收货人签字', '收货机构备注', '备注',
];
